...
window.loadDummyData = () => {
    closeModal();
    showToast('Generando datos dummy...', 'info');
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `seed-${monthNames[month].toLowerCase()}-${year}.json`;
    a.click();
    URL.revokeObjectURL(url);

    showToast(`Datos generados: L${balance.toLocaleString()} balance, ${transactions.length} transacciones`, 'success');
};
